...
function pwstore_import
    ...
    if $merge; and test -f $registry_path
        ...
        if test $status -ne 0
            echo "Failed to decrypt current registry"
            return 1
        end
        ...
        echo $merged_content | gpg --quiet --yes --recipient "Akarsh Jain" --encrypt --output $registry_path
        
        if test $status -ne 0
            echo "Failed to encrypt merged registry"
            return 1
        end
        ...
        if test $status -ne 0
            echo "Failed to import passwords"
            return 1
        end
